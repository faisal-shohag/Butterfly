      ...
      {bannerContent.map((slide, index) => (
        ...
        >
          <Image
            ...
            priority
            className={`transition-transform duration-1000 ease-in-out ${index === current ? 'scale-100' : 'scale-110'
              }`}
          />
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
            <div className="text-center text-white">
              <h2 className={`text-4xl font-bold mb-4 transition-all duration-1000 delay-300 ${index === current ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-10'
                }`}>
                {slide.title}
              </h2>
              <p className={`text-xl mb-8 transition-all duration-1000 delay-500 ${index === current ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                }`}>
                {slide.description}
              </p> 
